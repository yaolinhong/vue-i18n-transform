        ...
        var processedTemplate = templateString.replace(/`([^`]*)`/gim, function(_, templateContent) {
            ...
            while (i < templateContent.length) {
                ...
                if (char === '$' && templateContent[i + 1] === '{') {
                    ...
                    // 检查变量表达式中是否包含中文字符串
                    var hasChineseInVar = /(['"])((?:(?!\1)[^\\]|\\.)*[\u4e00-\u9fa5](?:(?!\1)[^\\]|\\.)*)(\1)/g.test(varContent);
                    
                    // 递归处理变量表达式中的字符串，但仅当包含中文时
                    var processedVarContent = hasChineseInVar ? varContent.replace(/(['"])((?:(?!\1)[^\\]|\\.)*)(\1)/g, function(stringMatch, quote, content, endQuote) {
            ...
            const finalProcessedContent = '`' + segments.join('') + '`';
