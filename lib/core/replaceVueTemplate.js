    ...
    return content.replace(/<template(.|\n|\r)*template>/gim, function (match) {
        ...
        match = match.replace(/`([^`]*)`/gim, function (fullMatch, templateContent) {
            ...
            if (templateContent.includes('___LANG_TEMP_PLACEHOLDER_') ||
                templateContent.includes('/*comment_') ||
                templateContent.includes('$lang(')) {  // 添加对$lang的检测
                return fullMatch;
            }

            // 检查是否包含中文字符，如果不包含则直接返回原始模板字符串
                ...
                if (char === '$' && templateContent[i + 1] === '{') {
                    ...
                    if (currentSegment) {
                        if (currentSegment.trim() && /[\u4e00-\u9fa5]/.test(currentSegment) && !isUrl(currentSegment.trim())) {
                            ...
                            if (currentSegment.trim().includes('$lang(')) {
                                comments[langComment] = currentSegment.trim();
                            } else {
                    ...
                    varContent = varContent.replace(/(["'])([^"']*[\u4e00-\u9fa5][^"']*)\1/g, function (match, quote, chineseText) {
                        // 已经包含$lang或是URL，直接跳过
                        if (isUrl(chineseText.trim()) || chineseText.includes('$lang(')) {
                            return match;
                        }
        ...
        match = match.replace(/((\w+-){0,}\w+=['"]|>|'|")([^'"<>]*[\u4e00-\u9fa5]+[^'"<>]*)(['"<])/gim, function (_, prev, __, match, after, offset) {
            ...
            // 添加对$lang的检测，如果已经包含$lang调用，直接返回
        ...
        // 换回注释、require、模板字符串
        var finalMatch = match.replace(/___LANG_TEMP_PLACEHOLDER_\d+___/gim, function (match) {
...
// 在处理模板字符串或其他地方添加以下检测函数
function processLangCall(text) {
    // 检测是否已经包含$lang调用或是嵌套的$lang结构
    if (text.includes('$lang(')) {
        // 如果是嵌套形式如 $lang('${$lang(...)}')，提取内部的$lang调用
        if (text.match(/\$lang\(['"`]\${?\$lang\(/)) {
            return text.replace(/\$lang\(['"`]\${?\$lang\(([^)]+)\)[}`]?['"`]\)/g, '$lang($1)');
        }
        // 如果只是普通的$lang调用，保持不变
        return text;
    }

    // 不包含$lang调用的普通文本，正常处理
    return text;
}
